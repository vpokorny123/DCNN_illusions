...
size = (224,224)
center_radii = np.arange(1,size[0]/2,2)
...
legend_names = ['Layer %s: Filter %s'%(layer,filter_num)]
plt.legend(legend_names, framealpha=.1)
plt.xlabel('Center Size (radius)')
plt.ylabel('Filter Activation')
plt.title('Example Size Tuning Response')
...
plt.savefig('%s_%s.png' %(layer, filter_num ), transparent = True, dpi = 300)
plt.show()

#save out max activation radius
#max_radius = center_radii[np.argmax(activation)]
