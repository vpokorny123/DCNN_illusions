...
center_amplitude = 2
max_surround_amp = 5
surround_amplitude = np.arange(0,max_surround_amp,.1)
...
if filter_num == 'all':
    with open("jsons/contrast_gsf_results_layer%s_filterall.txt" 
          %(layer), "r") as fp:
    # Load the dictionary from the file
        contrast_gsf_results = json.load(fp)
else:
...
center_radius = contrast_gsf_results #
rf = rfs[layer]
...
activation = []
y_error = []
for seed, delta in enumerate(surround_amplitude): #np.arange(-100,101,2): #[0,180]: #np.arange(0,541):
    if keep_same_seed ==1:
        seed = 1 
